...
说明沉浸式翻译当前仅支持"双语显示/隐藏"二态切换的局限性，以及三种显示模式（原文版、翻译版、混合版）的实现方案。核心原则：零 DOM 篡改不变，通过 Overlay 定位策略和 CSS 切换实现三模式。

### 当前结论

#### 已确定

- 当前 ImmersiveOverlay 仅支持两种状态：显示（双语——翻译在原文下方）和隐藏（CSS `visibility: hidden`）。用户无法只看翻译不看原文。
- TranslateControl 的 toggle 按钮对应 `immersiveOverlay.toggleAll()`，只做全局 CSS visibility 切换，无模式概念。
- 行业标杆（沉浸式翻译 Chrome 扩展、Google Translate 页面翻译）均支持至少三种显示模式：仅原文、仅翻译、双语对照。
- 当前 ImmersiveOverlay 的 `_positionEntry()` 使用 `rect.bottom + scrollY` 将翻译定位在原文正下方；若改为 `rect.top + scrollY` 并匹配高度和不透明背景，可视觉替换原文。

#### 合理假设

- **翻译版模式**的最优实现是将 Overlay 条目重定位到原文元素正上方（`rect.top` 而非 `rect.bottom`），使用不透明背景 + 匹配宽高覆盖原文。这不修改原始 DOM（零篡改原则不变），仅改变 Overlay 元素的 CSS 位置和样式。
- 不透明背景使用白色（`#fff`），在深色页面可能不完美，但覆盖绝大多数网页场景。未来可扩展为自动检测页面背景色。
- 三模式切换 UI 使用分段控制器（Segmented Control）替代现有 toggle 按钮，提供直观的模式选择体验。
- 模式切换应是纯 Chrome 侧操作（不需要重新翻译），延迟应在毫秒级。

#### 待确认

- 翻译版模式的不透明背景色是否需要自动适配页面主题色（当前先用白色，后续视反馈决定）。
- 是否需要记住用户上次选择的显示模式（当前默认始终从"混合版"开始）。

### 用户与场景

- 用户在阅读英文页面时触发沉浸式翻译，初始显示双语对照
- 用户想专注阅读中文译文，切换到"翻译版"模式——页面上的英文被译文视觉替换
- 用户想对比某段翻译质量，切换回"混合版"——原文与译文同时可见
- 用户想暂时关闭翻译恢复原始页面，切换到"原文版"
- 三种模式之间可任意切换，无需重新翻译
...
- ImmersiveOverlay 新增 `DisplayMode` 类型（`'bilingual' | 'original' | 'translated'`）和 `setDisplayMode()` 方法
- 翻译版模式：Overlay 条目重定位到原文位置 + 不透明背景覆盖
- action-executor.ts `executeInjectBilingual` 新增 `setDisplayMode` 操作模式
- TranslateControl.tsx toggle 按钮替换为三模式分段控制器
- VSCode 侧 browser-tools.ts 更新 `browser_inject_bilingual` Schema

#### 本次不做

- 自动检测页面背景色适配翻译版模式
- 记住用户显示模式偏好（跨会话持久化）
- 按段落级别混合模式（如部分段落显示双语、部分仅翻译）

### 关键体验

1. 用户触发沉浸式翻译后，默认显示混合版（双语对照，与现有行为一致）
2. 翻译完成后出现三个模式切换按钮：原文 | 双语 | 译文
3. 点击"译文"：原文位置被翻译文本覆盖，页面视觉上变成全翻译
4. 点击"双语"：恢复当前双语模式，翻译在原文下方
5. 点击"原文"：隐藏所有翻译，恢复原始页面
6. 模式切换毫秒级完成，无需重新调用 LLM

### 当前状态

- ImmersiveOverlay 仅支持显示/隐藏二态切换
- TranslateControl 提供 toggle（显示/隐藏）和 clear（清除）按钮
- 无"仅翻译版"模式能力

---

## 深度调研能力 — 类 Gemini Deep Research（evo_v35）

### 文档目标

说明如何将现有的简单报告生成器升级为自主迭代式深度调研引擎，使用户在 Chrome Side Panel 中发起一个研究主题后，Agent 能自主多轮搜索、阅读、推理、补缺，最终产出带引用的结构化报告。对标 Google Gemini Deep Research 的核心体验。

### 当前结论

#### 已确定

- 系统已有 `report-generator.ts`，实现了基础的"规划 URL → 逐页探索 → 生成报告"三段式流程，但存在明显局限：① 单轮 URL 规划，无迭代补缺能力；② 仅依赖 MCP 工具，未接入更丰富的 `browser_*` 工具；③ 无可编辑的研究计划；④ 报告无结构化引用系统；⑤ Chrome 侧无专属 UI，无法展示调研过程。
- 现有 `browser_*` 工具已足够支撑深度调研的核心循环：`browser_navigate` 导航、`browser_get_text` 提取页面全文、`browser_get_links` 获取链接、`browser_screenshot` 截图取证。
- WebSocket 协议已支持 `report_progress` / `report_result` 两个消息类型，可以在此基础上扩展更细粒度的调研协议。
- Chrome 侧已有 Tab 切换（Chat / Skills / Debug）的 UI 模式，可新增 Research Tab 或复用 Chat Tab 内的专属视图。

#### 合理假设

- 深度调研的核心差异在于**迭代式研究循环**：每轮搜索-阅读后 LLM 评估知识缺口，如果有未回答的子问题则自动发起新一轮搜索，最多迭代 3 轮、总计探索不超过 15 个页面。
- 用户在发起调研后，应先看到 Agent 生成的**研究计划**（拆解的子问题 + 搜索策略），可以编辑/确认后再开始执行，这是 Gemini Deep Research 的关键差异化体验。
- 调研过程中需要**实时思考流**推送给 Chrome 侧——Agent 当前在做什么、学到了什么、下一步计划是什么——让用户在 5-10 分钟的等待中有持续的信息反馈。
- 最终报告需要**结构化引用系统**：每个核心论点标注来源（`[1]`, `[2]`），报告末尾附完整参考文献列表，每条引用包含页面标题和 URL。
- 从 Chrome 侧应能一键导出报告为 `.md` 或 `.html` 文件下载到本地。

#### 待确认

- 迭代轮次上限（3 轮）和页面数上限（15 页）是否需要根据不同模型的 token 限制动态调整。
- 是否需要支持 Google 搜索集成（当前方案使用 `browser_navigate` 导航到搜索引擎页面 + `browser_get_links` 提取搜索结果，非 API 调用）。
- 研究计划的编辑粒度：用户可以添加/删除/修改子问题，还是只能整体确认/拒绝。当前按"可编辑子问题列表"方案实施。

### 用户与场景

- **场景 A：竞品分析** — 用户说"帮我调研 Cursor vs Windsurf vs Claude Code 的对比"，Agent 拆出技术架构、定价、用户评价等子维度，逐个搜索阅读后产出对比报告。
- **场景 B：技术选型** — 用户说"调研 React Server Components 的最新进展和最佳实践"，Agent 搜索官方文档、社区讨论、案例分析，产出带引用的技术摘要。
- **场景 C：基于当前页面深入调研** — 用户在一篇博客上说"深入调研这篇文章提到的所有观点"，Agent 以当前页面为起点，追踪文中链接和相关搜索，产出扩展报告。
- **场景 D：市场/行业调研** — 用户说"调研 2026 年 AI Agent 市场格局"，Agent 多轮搜索行业报告、融资新闻、产品发布，产出综合分析。

### 功能范围

#### 本次要做

- 重构 `report-generator.ts` 为 `DeepResearchEngine`：支持迭代式研究循环（plan → search → read → reason → gap-detect → re-plan）
- 研究计划数据模型：子问题列表、搜索策略、可编辑确认流程
- 引用追踪系统：每个信息点绑定来源 URL + 页面标题 + 原文摘录
- WebSocket 深度调研协议：research_plan / research_thinking / research_progress / research_report 等消息类型
- Chrome 深度调研 UI：研究计划编辑视图 + 实时思考流面板 + 报告渲染 + 导出按钮
- `/research <主题>` 斜杠命令入口
- `deep_research` 预设 Skill

#### 本次不做

- 外部搜索 API 集成（Google Search API / Bing API），当前通过浏览器导航到搜索页面实现
- 多模态输入（PDF、图片作为调研输入）
- 调研结果的跨会话引用和知识库积累
- 并发页面探索（保持顺序探索，避免 tab 管理复杂度）

### 关键体验

1. 用户输入 `/research AI Agent 市场格局 2026` 或在对话中说"帮我深度调研…"
2. 3-5 秒后，Chrome 侧显示 Agent 生成的研究计划：4-6 个子问题 + 初始搜索策略
3. 用户可以编辑（添加/删除/修改子问题），确认后 Agent 开始执行
4. 执行过程中，Chrome 侧实时显示思考流："正在搜索 AI Agent 融资动态…"、"发现 3 个关键信息源…"、"子问题 2/5 已完成，发现新的知识缺口…"
5. 每轮迭代结束后，如果有未回答的子问题，自动发起下一轮
6. 5-10 分钟后，产出结构化报告：目录、摘要、分主题论述（每段带 `[1][2]` 引用标注）、参考文献列表
7. 用户可以一键导出为 .md 或 .html
8. 用户可以追问"展开报告中关于定价的部分"进行迭代优化

### 当前状态

- `report-generator.ts` 提供基础三段式流程，无迭代能力
- Chrome 侧无深度调研专属 UI
- 无结构化引用系统
- 无研究计划编辑能力
- 无实时思考流推送
