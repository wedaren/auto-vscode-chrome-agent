...
- [~] 不考虑时间，使用最优解，优化 沉浸式翻译，避免篡改原有的 html 布局（已拆解为 evo_v33_001, evo_v33_002, evo_v33_003, evo_v33_004, evo_v33_005 — 零 DOM 篡改：ImtElementRegistry 内存注册表 + ImmersiveOverlay 绝对定位层 + extractParagraphs 零属性提取 + injectBilingual Overlay 注入 + 全链路验收）
- [x] 不考虑时间，使用最优解，沉浸式翻译能提供原文版与翻译版，或者混合版（已拆解为 evo_v34_001, evo_v34_002, evo_v34_003, evo_v34_004, evo_v34_005 — 三种显示模式：ImmersiveOverlay DisplayMode 引擎 + action-executor setDisplayMode + TranslateControl 三模式分段控制器 + VSCode Schema 扩展）
- [x] 不考虑时间，使用最优解，希望能做一些深度调研的工作，类似 google 的 gemini 的深度调研（已拆解为 evo_v35_001, evo_v35_002, evo_v35_003, evo_v35_004, evo_v35_005 — 深度调研体系：DeepResearchEngine 迭代式研究循环 + WebSocket 调研协议 + Chrome ResearchPanel UI + 结构化引用报告导出 + /research 斜杠命令）
