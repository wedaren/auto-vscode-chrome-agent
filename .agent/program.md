...
- [~] 不考虑时间，使用最优解，vsocde 插件可以配置默认模型，隐藏模型等，浏览器默认提供 5 个下拉选（已拆解为 evo_v24_001, evo_v24_002, evo_v24_003, evo_v24_004, evo_v24_005 — 模型管理配置：VSCode 配置 Schema + LmService 过滤/默认模型 + MessageHandler 配置传递 + Chrome 下拉 5 项限制）