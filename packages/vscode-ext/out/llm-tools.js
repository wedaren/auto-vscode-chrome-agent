...
exports.resolveTextsFromArgs = resolveTextsFromArgs;
exports.extractTextsFromValue = extractTextsFromValue;
/** 参数名别名优先级：texts > paragraphs > input（fallback 链） */
const TEXT_ARG_ALIASES = ['texts', 'paragraphs', 'input'];
// ────────────────────────────────────────────────────────────────
// 工具注册表
// ────────────────────────────────────────────────────────────────
...
function extractTextsFromValue(value) {
    ...
    if (Array.isArray(value)) {
        return value.map((item) => typeof item === 'string' ? item : (item && typeof item === 'object' && 'text' in item)
            ? String(item.text)
            : JSON.stringify(item));
    }
    // ── 字符串类型：可能是 JSON 或纯文本 ──
 ...
 *   - { texts: ["a","b"] }
 */
function extractTextsFromParsed(parsed) {
    // 顶层数组
    if (Array.isArray(parsed)) {
        return parsed.map((item) => typeof item === 'string' ? item : (item && typeof item === 'object' && 'text' in item)
            ? String(item.text)
            : JSON.stringify(item));
    }
    if (parsed && typeof parsed === 'object') {
        ...
        if (Array.isArray(obj.paragraphs)) {
            return obj.paragraphs.map((p) => (typeof p === 'string' ? p : (p && typeof p === 'object' && 'text' in p) ? String(p.text) : JSON.stringify(p)));
        }
        // { texts: [...] }
        if (Array.isArray(obj.texts)) {
            return obj.texts.map(String);
        }
    }
    return null;
}
