...
class AgentLoop {
    ...
    static TOTAL_TIMEOUT_MS = 5 * 60 * 1000;
    constructor(lmService, mcpClient, outputChannel, browserToolProvider, skillRegistry, skillRunner) {
     ...
     * @returns 最终答案 + 完整步骤记录
     */
    async run(userMessage, options = {}, token) {
        ...
        let roundCount = 0;
        let timedOut = false;
        this.outputChannel.appendLine(`[AgentLoop] 开始执行，maxSteps=${maxSteps}, totalTimeout=${totalTimeout}ms, 用户消息: ${userMessage.substring(0, 100)}`);
        // ── 总超时保护：超时后通过 CancellationTokenSource 中断循环 ──
        const timeoutCts = new vscode.CancellationTokenSource();
        const timeoutTimer = setTimeout(() => {
            timedOut = true;
            timeoutCts.cancel();
            this.outputChannel.appendLine(`[AgentLoop] 总超时 (${totalTimeout}ms) 已触发，中断循环`);
        }, totalTimeout);
        ...
        try {
            ...
            const agentSystemPrompt = this.buildAgentSystemPrompt(toolsDescription, options.systemPrompt);
            // 累积对话消息（LLM 可看到完整上下文）
            const messages = [
            ...
            while (roundCount < maxSteps) {
                ...
                this.outputChannel.appendLine(`[AgentLoop] LLM 输出 (${llmOutput.length} chars):\n${llmOutput.substring(0, 500)}`);
                ...
                if (parsed.type === 'FINAL_ANSWER') {
                    ...
                    this.outputChannel.appendLine(`[AgentLoop] 完成，共 ${roundCount} 轮，最终答案长度: ${parsed.answer.length}`);
                ...
                if (parsed.type === 'ACTION') {
                    ...
                    // Act 步骤
                    const actStep = this.createStep(roundCount, 'act', `调用工具 ${parsed.toolName}`, parsed.toolName, parsed.toolArgs);
                    steps.push(actStep);
                    onStep?.(actStep);
                    // 执行 MCP 工具调用
                ...
                steps.push(thinkStep);
                onStep?.(thinkStep);
                messages.push(vscode.LanguageModelChatMessage.User('Please respond using the required format: either ACTION with ACTION_INPUT, or FINAL_ANSWER.'));
            }
            // ── 达到 maxSteps 或超时，强制结束 ──
            const reason = timedOut
                ? `总超时 ${totalTimeout}ms`
                : `最大步数 ${maxSteps}`;
            this.outputChannel.appendLine(`[AgentLoop] 因 ${reason} 强制结束`);
     ...
     */
    async callLlm(model, messages, token) {
        let localCts;
    ...
    async getToolDescriptions() {
        ...
        if (this.mcpClient.connected) {
            try {
                ...
                this.outputChannel.appendLine(`[AgentLoop] 加载 ${mcpTools.length} 个 MCP 工具（去重后保留 ${mcpCount} 个）`);
            }
            catch (err) {
        ...
        if (this.skillRegistry && this.skillRunner) {
            ...
            if (enabledSkills.length > 0) {
                const skillList = enabledSkills
                    ...
                    .join('\n');
                lines.push(`- run_skill: 执行预定义 Skill（多步骤浏览器操作序列）。参数: {"skill_name": "技能名称", "params": {"参数名": "值"}}。可用 Skill:\n${skillList}`);
                this.outputChannel.appendLine(`[AgentLoop] 注册 run_skill 工具，${enabledSkills.length} 个可用 Skill`);
            }
        }
        if (lines.length === 0) {
        ...
        this.outputChannel.appendLine(`[AgentLoop] 共加载 ${lines.length} 个工具（browser: ${browserCount}, mcp: ${mcpCount}）`);
        return lines.join('\n');
    }
    /**
     ...
     */
    formatMcpToolSignature(name, description, inputSchema) {
        ...
        // 必填参数集合
        const requiredSet = new Set(Array.isArray(inputSchema.required)
...
6. **General principle** — Never give up after a single failure. Try an alternative approach (different selector, different tool, different strategy) before reporting failure to the user`;
     ...
     */
    parseLlmOutput(output) {
        // 提取 THOUGHT
        const thoughtMatch = output.match(/THOUGHT:\s*([\s\S]*?)(?=(?:ACTION:|FINAL_ANSWER:)|$)/i);
        ...
        if (actionMatch) {
            ...
            let toolArgs = {};
            const inputMatch = output.match(/ACTION_INPUT:\s*([\s\S]*?)(?=(?:THOUGHT:|ACTION:|FINAL_ANSWER:)|$)/i);
            if (inputMatch) {
                try {
                    ...
                    this.outputChannel.appendLine(`[AgentLoop] ACTION_INPUT JSON 解析失败，原文: ${inputMatch[1].substring(0, 200)}`);
                }
            }
            return {
                type: 'ACTION',
     ...
     */
    async executeRunSkill(toolArgs) {
        const skillName = toolArgs.skill_name;
        const params = toolArgs.params ?? {};
        this.outputChannel.appendLine(`[AgentLoop] run_skill: skill_name=${skillName}, params=${JSON.stringify(params)}`);
        if (!skillName) {
    ...
    summarizeSteps(steps) {
        ...
        if (observeSteps.length > 0) {
            parts.push(`已收集 ${observeSteps.length} 条工具观察结果。`);
        }
        return parts.join('\n\n');
    }
}
