...
class AgentLoop {
    ...
    async run(userMessage, options = {}, token) {
        ...
        const steps = [];
        let roundCount = 0;
        this.outputChannel.appendLine(`[AgentLoop] 开始执行，maxSteps=${maxSteps}, 用户消息: ${userMessage.substring(0, 100)}`);
        // 获取可用 MCP 工具描述
        ...
        // ── 达到 maxSteps 上限，强制结束 ──
        this.outputChannel.appendLine(`[AgentLoop] 达到最大步数 ${maxSteps}，强制结束`);
        const fallbackAnswer = `(Agent 达到最大步数 ${maxSteps} 限制后自动结束)\n\n` +
