...
function activate(context) {
    ...
    const uncaughtHandler = (err) => {
        outputChannel.appendLine(`[BrowserAgent][CRITICAL] uncaughtException: ${err.message}\n${err.stack ?? ''}`);
        vscode.window.showErrorMessage(`Browser Agent 遇到未捕获异常: ${err.message}`);
    };
    const rejectionHandler = (reason) => {
        const msg = reason instanceof Error ? reason.message : String(reason);
        outputChannel.appendLine(`[BrowserAgent][CRITICAL] unhandledRejection: ${msg}`);
        vscode.window.showErrorMessage(`Browser Agent 遇到未处理的 Promise 异常: ${msg}`);
    };
    ...
    void (async () => {
        try {
            ...
            outputChannel.appendLine(`[BrowserAgent][CRITICAL] WebSocket 服务初始化失败，标记为不健康: ${errMsg}`);
            vscode.window.showErrorMessage(`Browser Agent WebSocket 启动失败，消息通道不可用: ${errMsg}`);
        }
        ...
        if (wsServerHealthy) {
            ...
            outputChannel.appendLine('[BrowserAgent] MessageHandler 未注册：WebSocket 不健康，已阻断下游依赖');
        }
    })();
    // 初始化报告生成器
    ...
    context.subscriptions.push(outputChannel, configChangeDisposable, {
        dispose: () => {
