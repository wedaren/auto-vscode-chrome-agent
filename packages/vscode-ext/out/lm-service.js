...
class LmService {
    ...
    getModelPreferences() {
        ...
        return {
            defaultModelId: config.get('defaultModelId', ''),
            maxVisibleModels: config.get('maxVisibleModels', 5),
        };
    }
    /**
     * 列出所有可用的语言模型信息
    ...
    async listModels() {
        ...
        const config = vscode.workspace.getConfiguration('browserAgent.models');
        const hiddenModelIds = config.get('hiddenModelIds', []);
        const filtered = hiddenModelIds.length > 0
            ? allModels.filter((m) => !hiddenModelIds.includes(m.id))
            : allModels;
        this.outputChannel.appendLine(`[LmService] 发现 ${allModels.length} 个可用模型，隐藏 ${allModels.length - filtered.length} 个，返回 ${filtered.length} 个`);
