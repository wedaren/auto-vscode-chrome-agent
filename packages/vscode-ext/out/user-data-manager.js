...
/** init() 自动创建的子目录列表 */
const SUB_DIRS = ['skills', 'config', 'sessions'];
// ────────────────────────────────────────────────────────────────
// UserDataManager 类
// ────────────────────────────────────────────────────────────────
/**
...
class UserDataManager {
    ...
    _onDidChangeRoot = new vscode.EventEmitter();
    ...
    async init() {
        ...
        try {
            ...
            this.outputChannel.appendLine(`[UserDataManager] 数据目录已初始化: ${this.rootDir}`);
            this.outputChannel.appendLine(`[UserDataManager] 子目录: ${SUB_DIRS.join(', ')}`);
            this._onDidChangeRoot.fire(this.rootDir);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            this.outputChannel.appendLine(`[UserDataManager] 初始化目录失败: ${message}`);
            vscode.window.showErrorMessage(`Browser Agent: 无法创建数据目录 ${this.rootDir} — ${message}`);
        }
    }
    /**
     * 获取数据目录下的绝对路径
    ...
    async readJSON(...segments) {
        ...
        try {
            ...
            if (isNodeError(err) && err.code === 'ENOENT') {
                this.outputChannel.appendLine(`[UserDataManager] 文件不存在（首次使用）: ${filePath}`);
                return undefined;
            }
            const message = err instanceof Error ? err.message : String(err);
            this.outputChannel.appendLine(`[UserDataManager] 读取 JSON 失败: ${filePath} — ${message}`);
            return undefined;
        }
    }
    /**
    ...
    async writeJSON(data, ...segments) {
        ...
        try {
            ...
            this.outputChannel.appendLine(`[UserDataManager] 已写入: ${filePath}`);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            this.outputChannel.appendLine(`[UserDataManager] 写入 JSON 失败: ${filePath} — ${message}`);
     ...
     */
    static getConfiguredDir() {
        return vscode.workspace
            .getConfiguration('browserAgent')
            .get('userDataDir', DEFAULT_DIR);
    }
    /**
     * 解析目录路径：展开 ~ 为用户 home 目录
     */
    static resolveDir(dir) {
...
//# sourceMappingURL=user-data-manager.js.map