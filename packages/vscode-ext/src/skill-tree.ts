...
import { SkillRunner, SkillProgress } from './skill-runner';
      ...
      try {
        const result = await skillRunner.execute(
          ...
          (sp: SkillProgress) => {
            ...
            outputChannel.appendLine(
              `[SkillTree] 进度: ${pct}% - ${sp.description} (${sp.status})`,
            );
          },
          cts.token,
        );
        ...
        if (result.success) {
          vscode.window.showInformationMessage(
            `Skill "${skillRef.displayName}" 执行成功（${result.stepResults.length} 步）`,
          );
        } else {
          vscode.window.showWarningMessage(
            `Skill "${skillRef.displayName}" 执行失败`,
          );
        }
