...
export type McpConnectionState =
  ...
  | 'connecting'     // 正在连接中
  | 'connected'      // 已连接且健康
  | 'reconnecting'   // 崩溃后正在重连
  | 'disconnected';  // 已断开（重试耗尽或手动断开）
...
export class McpClient {
  ...
  private _connectionState: McpConnectionState = 'idle';
  ...
  async listTools(): Promise<McpToolInfo[]> {
    ...
    this.log(
      `可用工具 (${tools.length}): ${tools.map((t) => t.name).join(', ')}`,
    );
  ...
  async callTool(toolName: string, args: Record<string, unknown> = {}): Promise<McpToolResult> {
    ...
    this.log(
      `调用工具: ${toolName}, 参数: ${JSON.stringify(args)}`,
    );

    const result = await this.client.callTool({ name: toolName, arguments: args });

    this.log(
      `工具 ${toolName} 返回: ${JSON.stringify(result).substring(0, 500)}`,
    );
  ...
  private scheduleReconnect(): void {
    if (this.retryCount >= MAX_RESTART_ATTEMPTS) {
      this.log(
        `[LIFECYCLE] 已达最大重启次数 (${MAX_RESTART_ATTEMPTS})，放弃自动重连`,
      );
      this.setConnectionState('disconnected');
      vscode.window.showWarningMessage(
        `Browser Agent: DevTools MCP 子进程崩溃且重启 ${MAX_RESTART_ATTEMPTS} 次均失败，请手动重新连接`,
      );
      return;
    }
    ...
    this.log(
      `[LIFECYCLE] 将在 ${delay}ms 后进行第 ${this.retryCount}/${MAX_RESTART_ATTEMPTS} 次重连`,
    );
    ...
    this.reconnectTimer = setTimeout(() => {
      ...
      void this.doConnect().catch((err) => {
        this.log(
          `[LIFECYCLE] 重连失败: ${err instanceof Error ? err.message : String(err)}`,
        );
      });
    }, delay);
  }
  ...
  private async cleanupConnection(): Promise<void> {
    if (this.client) {
      try {
        ...
        this.log(
          `关闭 Client 时出错: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
      this.client = null;
    }
