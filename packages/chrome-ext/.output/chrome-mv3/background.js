          ...
          if (action.type === "navigate" && action.url) {
            browser.tabs.query({ active: true, currentWindow: true }).then((tabs) => {
              ...
              browser.tabs.update(tabId, { url: action.url }).then(() => {
                sendResponse({
                  type: "ACTION_RESULT",
                  payload: { success: true, data: { navigated: action.url } }
                });
              }).catch((err) => {
                sendResponse({
                  type: "ACTION_RESULT",
                  payload: { success: false, error: `导航失败: ${err instanceof Error ? err.message : String(err)}` }
                });
              });
            });
            return true;
